...
class Home extends Component{
    ...
    async componentDidMount(){
        try{
            let res = await axios.get("https://covid19.mathdro.id/api/countries/india/confirmed");
            for(var i=0;i<11;i++){
                if(res.data[i].provinceState==='Tamil Nadu'){
                    let confirme = res.data[i].confirmed.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
                    let recovere = res.data[i].recovered.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
                    let activ = res.data[i].active.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
                    let deaths = res.data[i].deaths.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
                    this.setState({confirmed:confirme,recovered:recovere,death:deaths,active:activ});
                    break;
                }
            }
        }catch (e){
